...
- APIキー、アクセストークン、Client Secret、OAuth Secret、Webhook Secret、秘密鍵、Service Account Keyを保存しない
- APIの利用コードや公開仕様は保存できるが、実際の秘密値、環境変数ファイル、認証ヘッダー、秘密を含むログや設定ファイルは保存しない
- 秘密値はGitHubへ一度もコミットしない。誤って含めた場合は削除だけで済ませず、直ちにキーを無効化・再発行する
